...
validUpdates.forEach(calculateMidPageNumScore);
...
function getValidRules(data, index) {
  ...
  validateRules(validRules,data);
}

function validateRules(validRules, pages){

  let updateIsValid = true;
  let result = -1;
  let result2 = -1;

  for (let x = 0; x < validRules.length; x++) {
    rules = validRules[x].toString().split(",");

    result = pages.indexOf(rules[0]);
    result2 = pages.indexOf(rules[1]);

    if (result > result2){
      console.log("InValid Update rule!")
      updateIsValid = false;
      break;
    }
  }
