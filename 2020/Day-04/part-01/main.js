...
function fnValidatePassprt(passPort){
    var validParamNum = 0;
    var validCountry = "false";
    for (var x = 0; x < validParameters.length; x++) {
      if(passPort.includes(validParameters[x])){
        validParamNum += 1;
      }
    }

    if(passPort.includes("cid:")){
      validCountry = "true";
    }

    if((validCountry == "false" && validParamNum == 7) || (validCountry == "true" && validParamNum == 7)){
      return "true";
    }else {
      return "false";
    }
